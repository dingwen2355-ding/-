    ...
    <el-form size="mini" label-width="80px" @submit.native.prevent>
      <el-form-item label="表单标识">
        ...
        <el-select v-model="formKey" @change="updateElementFormKey">
          <el-option v-for="i in formObj" :key="i.dataId" :value="i.dataId" :label="i.formName + '|' + i.formCode" />
          <el-option label="无" value="" />
        </el-select>
      </el-form-item>
      <el-form-item label="业务标识">
        <el-input v-model="businessKey" clearable @change="updateElementBusinessKey" />
...
export default {
  ...
  methods: {
    resetFormList() {
      ...
      console.log(this.prefix,'prefixprefix');
      this.formData =
