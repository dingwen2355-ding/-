        ...
        <el-button-group key="file-control">
          ...
          <el-tooltip effect="light">
            <div slot="content">
              ...
              <br />
              <el-button :size="headerButtonSize" type="text" @click="saveProcess()">保存并部署</el-button>
              <br />
            </div>
            <el-button :size="headerButtonSize" :type="headerButtonType" icon="el-icon-download">下载或保存</el-button>
          </el-tooltip>
          <el-tooltip effect="light">
            <div slot="content">
...
// 自定义的属性解析文件
import SelfDescriptor from './plugins/descriptor/SelfDescriptor.json'
// 标签解析 Extension
...
export default {
  ...
  computed: {
    ...
    moddleExtensions() {
      ...
      Extensions.self = SelfDescriptor
      return Extensions
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async downloadProcess(type, name) {
      try {
        ...
        if (type === 'xml' || type === 'bpmn') {
          ...
          console.log(this.$store.state.bpmn.elementBusinessObject, ' this.$store.state.bpmn.elementBusinessObject ')
          var ebo = this.$store.state.bpmn.elementBusinessObject
          let flag = false
          if (ebo.flowElements && ebo.$type === 'bpmn:Process' && ebo.flowElements.length > 0) {
            flag = true
            //  alert(111)
          } else if (
            ebo.$type !== 'bpmn:Process' &&
            ebo.$type !== 'bpmn:Collaboration' &&
            ebo.$type !== 'bpmn:Participant'
          ) {
            flag = true
            //  alert(222)
          } else if (ebo.$type === 'bpmn:Participant') {
            alert(333)
            flag = true
          } else if (ebo.$type === 'bpmn:Collaboration' && ebo.di !== null) {
            // alert(444)
            flag = true
          }
          if (!flag) {
            return this.$message.warning('空白流程！！')
          }

          //保存并部署
          let prcessName = prompt('请输入流程文件名称', 'default')
          if (prcessName === null) return
          const { xml } = await this.bpmnModeler.saveXML()
          // let id = this.getQueryString("token");
          // console.log(xml);
          // console.log(id);
          const saveVo = {
            name: prcessName + '.bpmn',
            xml
          }

          postAction(this.requestUrl + '/v1/ActivitiDesignController/saveProcess', saveVo)
            .then(response => {
    ...
    //如果浏览器中有流程定义的Key则查询后加载到页面
    importProcessByid() {
      console.log(this.$route.query.id, 'params')
      this.dataId = this.$route.query.id
      // let id = this.getQueryString('proId')
      // if (id === null) return
      ...
      if (flag === '0') {
        this.addByProcessByid(id, token)
      } else {
        let url = this.requestUrl + '/v1/ActivitiDesignController/getProcessXml'
        // console.log(id, token);
        getAction(url + '?id=' + this.dataId)
          .then(response => {
            if (response.code == 200) {
    ...
    //根据相同的processId添加流程
    addByProcessByid(id, token) {
      let url = this.requestUrl + '/v1/ActivitiDesignController/getProcessXml'
      console.log('根据相同的processId添加流程')
      // console.log(id, token);
      axios
        .get(url + '?token=' + token + '&id=' + id)
        .then(response => {
          if (response.data.success) {
            let xml = response.data.result
            if (xml !== null) {
              // console.log(xml);
              this.createNewDiagram(xml)
            }
