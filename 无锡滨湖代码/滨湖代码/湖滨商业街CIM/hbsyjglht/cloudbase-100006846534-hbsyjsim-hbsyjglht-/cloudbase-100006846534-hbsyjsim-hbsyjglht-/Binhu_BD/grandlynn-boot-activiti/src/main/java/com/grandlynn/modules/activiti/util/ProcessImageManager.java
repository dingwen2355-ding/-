...
import com.grandlynn.modules.activiti.service.impl.image.CustomProcessDiagramGenerator;
...
public class ProcessImageManager {
    ...
    public InputStream getFlowImgByProcessDefId(String processDefId) throws Exception {
        ...
        try {
            ...
            throw new Exception("通过流程实例ID" + processDefId + "获取流程图时出现异常！", e);
        } finally {
            if (imageStream != null) {
                imageStream.close();
            }
...
//            获取历史节点
            List<HistoricActivityInstance> historicActivityInstanceList = historyService.createHistoricActivityInstanceQuery()
                    ...
                    .asc()
                    .list();
//            Set<String> collect = historicActivityInstanceList.stream().map(HistoricActivityInstance::getActivityId).collect(Collectors.toSet());
//            if (collect.size() != historicActivityInstanceList.size()) {
//                //有驳回
//                historicActivityInstanceList = handleReject(historicActivityInstanceList);
//            }
...
//            List<String> highLightedFlowsIds = getHighLightedFlows(bpmnModel, historicActivityInstanceList);
            List<String> highLightedFlowsIds = hightLine(procInstId, historicActivityInstanceList);
...
//                log.info("【完成】-匹配当前节点-ActivityId=【{}】需要高亮显示的流出线", currentActivityId);
    ...
    public List<String> hightLine(String id, List<HistoricActivityInstance> historicActivityInstances) {
        //获取历史流程实例
        ...
        //已经完成的任务集合
        List<HistoricActivityInstance> finish = new ArrayList<>();
/*        for (HistoricActivityInstance historicActivityInstance : historicActivityInstances) {
            if (historicActivityInstance.getEndTime() != null) {
                finish.add(historicActivityInstance);
            }
        }*/
        finish = historicActivityInstances;

        Set<String> flowIds = new TreeSet<>();

        loop:
        for (int i = 1, j = 0; i < finish.size(); i++, j++) {
            //前一个历史节点
            HistoricActivityInstance preHistory = finish.get(j);
            if ("parallelGateway".equals(preHistory.getActivityType()) ||
                    "inclusiveGateway".equals(preHistory.getActivityType())) {
                //如果是并行网关，单独处理
                handleParallelGateway(preHistory, finish, flowIds, sequenceFlows);
                continue;
            }
            //后一个历史节点
            HistoricActivityInstance postHistory = finish.get(i);
            //从map取出对应的flowid
            String flowId = map.get(preHistory.getActivityId() + postHistory.getActivityId());
            if (flowId != null && preHistory.getEndTime() != null) {
                flowIds.add(flowId);
            }
        }
        //手动处理第一个
        if (finish.size() == 1) {
            HistoricActivityInstance historicActivityInstance = finish.get(0);
            String activityId = historicActivityInstance.getActivityId();
            for (SequenceFlow sequenceFlow : sequenceFlows) {
                //如果源头为第一个的线都添加进去
                if (StrUtil.equals(sequenceFlow.getSourceRef(), activityId)) {
     ...
     * @return
     */
    private List<HistoricActivityInstance> handleReject(List<HistoricActivityInstance> historicActivityInstances) {
        Set<String> collect = historicActivityInstances.stream().map(HistoricActivityInstance::getActivityId).collect(Collectors.toSet());
        if (collect.size() == historicActivityInstances.size()) {
            //有驳回
            return new ArrayList<>(historicActivityInstances);
        }
        Stack<HistoricActivityInstance> stack = new Stack<>();
        //保存已经添加过的历史流程的activityId
        List<String> list = new ArrayList<>();
        for (HistoricActivityInstance historicActivityInstance : historicActivityInstances) {
            if (list.contains(historicActivityInstance.getActivityId())) {
                //循环删除到重复的位置
                while (true) {
                    String activityId = stack.pop().getActivityId();
                ...
                ;
                //添加正确的历史流程
                stack.push(historicActivityInstance);
            } else {
                stack.add(historicActivityInstance);
                list.add(historicActivityInstance.getActivityId());
            }
        }
        return handleReject(stack);
    }
     ...
     * @param sequenceFlows
     */
    private void handleParallelGateway(HistoricActivityInstance preHistory, List<HistoricActivityInstance> finish, Set<String> flowIds, List<SequenceFlow> sequenceFlows) {
        String activityId = preHistory.getActivityId();
        //获取当前网关下对应的节点
        List<SequenceFlow> targetFlows = sequenceFlows.stream()
                .filter(item -> StrUtil.equals(activityId, item.getSourceRef()))
                .collect(Collectors.toList());
        //遍历并行网关后已经完成的任务
        List<String> complementActivityIds = new ArrayList<>();
        for (SequenceFlow sequenceFlow : targetFlows) {
            finish.stream()
                    .filter(item -> StrUtil.equals(item.getActivityId(), sequenceFlow.getTargetRef()))
                    .findFirst()
                    //完成任务的node
                    .ifPresent(item -> {
                        //判断是否完成任务
                        if (item.getEndTime() != null) {
                            //记录完成后的线条
                            complementActivityIds.add(item.getActivityId());
                        }
                        flowIds.add(sequenceFlow.getId());
                    });
        }
        //遍历完成后的线条添加到高亮中
        for (String complementActivityId : complementActivityIds) {
            List<String> ids = sequenceFlows.stream()
                    .filter(item -> StrUtil.equals(item.getSourceRef(), complementActivityId))
                    .map(BaseElement::getId).collect(Collectors.toList());
            flowIds.addAll(ids);
        }
    }
}