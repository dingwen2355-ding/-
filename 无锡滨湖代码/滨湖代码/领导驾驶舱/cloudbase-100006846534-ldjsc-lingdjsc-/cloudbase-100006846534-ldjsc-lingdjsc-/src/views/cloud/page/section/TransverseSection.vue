    ...
    <div class="func-warp" v-show="showSetting">
      <template v-for="(item, index) in setting">
        <div :key="index" class="warp-list" v-if="item.type === 'slider'">
          ...
          <div class="list">
            <el-slider :min="item.min" :max="item.max" :step="item.step" v-model="item.value" @change="changeValue"
...
export default {
      ...
      handler(val) {
        if (val.MouseClickPoint) {
          this.setting.some((item) => {
            if (item.type === "slider") {
              item.value = val.MouseClickPoint[2];
  ...
  async mounted() {
    ...
    window.sealAPI._clip.start(0);
    this.$message.success("进入横剖切！");
  ...
  methods: {
    ...
    handleDownload() {
      const filename = "横剖切_" + new Date().getTime();
