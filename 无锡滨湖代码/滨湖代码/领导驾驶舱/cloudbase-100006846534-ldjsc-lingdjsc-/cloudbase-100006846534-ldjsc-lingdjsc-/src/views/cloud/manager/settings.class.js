class Settings {
     ...
     * @param {number} mask 掩码，请参考MousePickMask
     * @returns 可选的回调函数
     */
    setMousePickMask = function (mask) {
        return new Promise((resolve, reject) => {
            window.origAPI.settings.setMousePickMask(mask, () => {
                resolve()
            })
        })
    }
     ...
     * @param {boolean} bEnable 是否开启
     * @param {number} period 事件监听的间隔时间，值越小返回的越快，单位：毫秒，默认：20ms，取值范围：[0~100]
     * @returns 可选的回调函数
     */
    setEnableCameraMovingEvent = function (bEnable, period) {
        return new Promise((resolve, reject) => {
            window.origAPI.settings.setEnableCameraMovingEvent(bEnable, period, () => {
                resolve()
            })
        })
    }
