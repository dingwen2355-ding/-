    ...
    switch (data.eventtype) {
        case 'LeftMouseButtonClick':
            ...
            if(data.Type=="TileLayer"){
              let coor=[]
              if(data.BoundsMax&&data.BoundsMax.length==3&&data.BoundsMin&&data.BoundsMin.length==3){
                  coor = [calAvg(data.BoundsMax[0],data.BoundsMin[0]),calAvg(data.BoundsMax[1],data.BoundsMin[1])]
              }
              // if(data.MouseClickPoint&&data.MouseClickPoint.length==3){
              //     coor = [data.MouseClickPoint[0],data.MouseClickPoint[1]]
              // }
              if(coor.length==2){
                store.dispatch('SET_BUILDCOORD_ACTION', coor)
              }
            }else if(data.Type=="marker" ){
              if(data.GroupID=="vmMkrGroup"){
                store.dispatch('SET_VMKR_ACTION', {
                  id:data.Id,
                  coor:data.MouseClickPoint
                })
              }
            }
            break
...
SealAirCityAPI.prototype.initPlayer = function (instanceId, divId, ipAddress) {
    return new Promise((resolve, reject) => {
        ...
        let acp = new DigitalTwinPlayer(ipAddress, params)
