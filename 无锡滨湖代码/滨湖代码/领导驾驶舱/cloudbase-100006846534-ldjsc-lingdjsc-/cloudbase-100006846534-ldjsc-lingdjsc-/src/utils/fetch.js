...
export function get(url, data, needToken = true) {
  ...
  return axios.get(spliceURL(url), {
	headers : {
      'Freedo-Cim-Authorization': needToken?store.state.login.account.token:""
    },
    params: data
  })
}
...
export function post(url, data, needToken = true) {
  ...
  if (needToken){
	config.headers = {
      'Freedo-Cim-Authorization': store.state.login.account.token
    }
  }
  return axios(config)
}
...
axios.interceptors.response.use(
  ...
  (error) => {
    if(error.response&&error.response.status===401){
      if(error.response.config.url.indexOf("videoService/accounts/authorize") !== -1){
        console.log('大华鉴权')
      }else{
