<template>
    <view class="content">
        <div class="panel2" v-if="list.length > 0">
            <div class="item">
                <sally-tree :option="config" :list="list"></sally-tree>
            </div>
        </div>
...
import SallyTree from '../../plugins/sally-tree/sally-tree.vue'
export default {
    components: { uniTag, oEmpty, SallyTree },
    data() {
        return {
            page: {
                pageSize: 10,
    ...
    methods: {
        ...
        getData(cb) {
            ...
            this.$api.ListDept(this.page).then(r => {
                uni.hideLoading();
