<template>
    <view class="content">
        ...
        <div class="field">
            <div class="label">校园类型：</div>
            <div class="value">{{ getSelectType(row.selectType) }}</div>
        </div>
        <div class="field">
            <div class="label">学校性质：</div>
            <div class="value">{{ getSelectAttribute(row.attribute) }}</div>
        </div>
        <div class="field">
        ...
        <div class="field">
            <div class="label">法定代表：</div>
            <div class="value">{{ row.legalTel || '/' }}</div>
        </div>
        <div class="field">
        ...
        <div class="field">
            <div class="label">创建负责：</div>
            <div class="value" v-html="row.creator || '/'"></div>
        </div>
        <div class="field">
            <div class="label">联系方式：</div>
            <div class="value" v-html="row.creatorTel || '/'"></div>
        </div>
        <div class="field">
...
export default {
    ...
    onLoad(query) {
        this.$api.schoolDetail(query.id).then(res => {
            this.row = res.data
            this.files = [];
    ...
    methods: {
        getSelectType(val) {
            ...
            return dicts["data_civilization_school_type"].filter(d => d.value == val)[0].label
        },
        getSelectLevel(val) {
            if(val == null) return '/'
            let dicts = JSON.parse(window.localStorage.getItem("dicts"));
        ...
        getSelectAttribute(val) {
            let dicts = JSON.parse(window.localStorage.getItem("dicts"));
            return dicts["data_civilization_school_attribute"].filter(d => d.value == val)[0].label
        },
        goback() {
            uni.navigateBack()
        },
