<template>
    <view class="content">
        <div class="panel2" v-if="list.length > 0">
            <div :class="{ item: true, active: activeId == i.id }" v-for="i, index in list" :key="index"
                @click="toDetail(i)">
                <div class="top">
                    <!-- <image mode="aspectFit" v-if="i.attchpath" :src="$resource(i.attchpath)"></image>
                    <div v-else class="img">暂无附件</div> -->
                    <div class="text">
                        <div class="content">检查类型名称：{{ i.name }}</div>
                        <div>
                            <div>创建时间：{{ i.createTime }}</div>
                        </div>
                    </div>
                    <uni-icons class="icons" :type="activeId == i.id ? 'top' : 'bottom'" size="24"
                        color="#00000050"></uni-icons>
                </div>
                <div class="bottom" v-if="activeId == i.id">
                    <uni-tag v-for="j in list2" :key="j.id" class="tag" :inverted="true" :text="j.name" size="small"
                        type="success" />
                </div>
            </div>
        </div>
...
export default {
    components: { uniTag, oEmpty, uniIcons },
    data() {
        return {
            page: {
                pageSize: 10,
    ...
    methods: {
        toDetail(row) {
            ...
            if (this.activeId == row.id) {
                ...
                this.$api.ListcheckItem(this.activeId).then(r => {
                    this.list2 = r.data.records
                })
            }
        ...
        getData(cb) {
            ...
            this.$api.checkPointsTypePage(this.page).then(r => {
                uni.hideLoading();
...
.content {
    ...
    .panel2 {
        ...
        .active {
            border: 1px solid #2c7de7 !important;
