<template>
    <view class="content">
        <div class="field">
            <div class="label">村镇名称</div>
            <div class="value">{{ row.name || '/' }}</div>
        </div>
        <div class="field">
            <div class="label">村镇类型：</div>
        ...
        <div class="field">
            <div class="label">组织负责：</div>
            <div class="value">{{ row.partyLeader || '/' }}</div>
        </div>

        <div class="field">
            <div class="label">联系电话：</div>
            <div class="value">{{ row.partyTel || '/' }}</div>
        </div>
        <div class="field">
        ...
        <div class="field">
            <div class="label">联系电话：</div>
            <div class="value">{{ row.creatorTel || '/' }}</div>
        </div>
        <div class="field">
...
export default {
    ...
    onLoad(query) {
        this.$api.townDetail(query.id).then(res => {
            this.row = res.data
            this.files = [];
    ...
    methods: {
        ...
        getSelectType(val) {
            if(val == null) return '/'
            let dicts = JSON.parse(window.localStorage.getItem("dicts"));
            return dicts["data_civilization_town_type"].filter(d => d.value == val)[0].label
        },
        preview(index) {
            uni.previewImage({
