...
export default {
    ...
    data() {
        return {
            ...
            list: [
                // {
                //     content:"1eqweqwedascdnajsdakjsbdaskbdaksd",
                //     reportTime:"2022-03-11 12:55:44",
                //     timelimit:"2022-03-11 12:55:44",
                // }
            ],
            isA: true
        }
    ...
    methods: {
        ...
        toDetail(row) {
            if (this.isA) {
                ...
                uni.navigateTo({
                    url: "./../gongdan/handle?row=" + encodeURIComponent(str)+"&title=处置"
                });
            } else {
                let str = JSON.stringify(row)
                uni.navigateTo({
        ...
        getData(cb) {
            ...
            let api = this.isA ? this.$api.handle(this.page) : this.$api.handled(this.page);
            api.then(r => {
                uni.hideLoading();
