<template>
  <view class="custom-tree-select-content">
    <view :class="['select-list', { disabled }, { active: selectList.length }]" @click.stop="open">
      <view class="left">
      ...
      <view class="right">
        ...
        <uni-icons v-if="selectList.length && clearable" type="clear" size="24" color="#c0c4cc"
          @click.native.stop="clear"></uni-icons>
      </view>
    </view>
    <uni-popup v-if="showPopup" ref="popup" :animation="animation" :is-mask-click="isMaskClick"
      ...
      <view class="popup-content" :style="{ height: contentHeight }">
        <view class="title">
          <view v-if="mutiple && canSelectAll" class="left" @click.stop="handleSelectAll">
        ...
        <view v-if="search" class="search-box">
          ...
          <button type="primary" size="mini" class="search-btn" @click.stop="handleSearch(false)">
            搜索
          </button>
        </view>
        <view v-if="treeData.length" class="select-content">
          <scroll-view class="scroll-view-box" :scroll-top="scrollTop" scroll-y="true" @touchmove.stop>
            ...
            <data-select-item v-for="item in filterTreeData" :key="item[dataValue]" :node="item" :dataLabel="dataLabel"
              ...
              :linkage="linkage"></data-select-item>
            <view class="sentry" />
          </scroll-view>
        </view>
...
export default {
  ...
  components: {
    dataSelectItem, uniIcons, uniPopup, uniEasyinput
  },
  model: {
    prop: 'value',
    event: 'input'
