...
let data = ref([]);
onMounted(() => {
  $axios_get(IP + "/screen/api/civilization/familyList", {
      params: {
        pageNum: 1,
        pageSize: 10
      }
...
<template>
  ...
  <div class="detail-wmjt" v-if="showWMJT">
    <div class="close" @click="showWMJT = false">×</div>
    <div class="title">
      <div class="titlebg">{{ detail.name }}</div>
    </div>
    <div class="content">
      <el-descriptions>
        <el-descriptions-item label="家庭名称：" :span="3">{{
          detail.name
