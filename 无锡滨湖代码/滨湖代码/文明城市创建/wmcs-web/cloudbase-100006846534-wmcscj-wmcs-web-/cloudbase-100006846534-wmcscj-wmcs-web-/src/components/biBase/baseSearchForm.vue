<template>
    <div class="baseTableSearch" :class="classType">
        <!-- 搜索栏 -->
        <div v-if="!noSearch">
            <t-row>
                <t-col :span="noSearchBtn ? 12 : 10">
                    <slot name="customQuery" />
                </t-col>
                <t-col v-if="!noSearchBtn" :span="2" class="operation-container">
                    <t-button theme="primary" class="margin-right-10" @click="startS">查询</t-button>
                    <t-button ghost @click="resetQ">重置</t-button>
                    <!-- <t-button theme="primary" :style="{ marginLeft: '8px' }"> 查询 </t-button>
                    <t-button variant="base" theme="default"> 重置 </t-button> -->
                </t-col>
            </t-row>
        </div>
        <!-- <div class="search-wapper show-flex-r">
            ...
            <div class="search-button show-flex-r" style="margin-left:20px;margin-top:5px;">
                <t-button theme="primary" class="margin-right-10" @click="startS">查询</t-button>
                <t-button ghost @click="resetQ">重置</t-button>
            </div>
        </div> -->
        ...
        getAllQuery(searchRef) {
            ...
            for (let item in this.queryRef) {
                ...
                if (this.queryRef[item]['getAllSearchValue']) {
                    // 所有查询组件依序组装查询条件
                    let query_data = this.queryRef[item]['getAllSearchValue']()
                    Object.assign(searchData, query_data)
