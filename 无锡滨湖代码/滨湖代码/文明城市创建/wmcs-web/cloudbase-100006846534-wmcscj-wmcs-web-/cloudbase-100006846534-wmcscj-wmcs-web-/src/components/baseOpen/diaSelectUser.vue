                    ...
                    <baseSearchForm ref="searchTable" @getList="getList" @search="startSearch" @reset="resetQuery">
                        <div slot="customQuery" class="listSearch">
                            <div class="search-line">
                                ...
                                <normalSearch ref="account" labelName="登录账号" keyName="account" placName="请输入登录账号" />
                                <normalSearch ref="majorMobile" labelName="手机号码" keyName="majorMobile" placName="请输入手机号码" />
                            </div>
                        </div>
                        <div slot="table">
                            <!-- 已经选择的 -->
                            ...
                            <el-table v-loading="tableListLoding" stripe size="mini" :data="tableData">
                                <el-table-column label="用户账号" prop="account" width="200" />
                                <el-table-column label="用户名称" prop="name" min-width="200" />
                                <el-table-column label="手机号码" prop="majorMobile" width="200" />
                                <el-table-column label="用户部门" prop="majorDept" width="200" />
                                <el-table-column label="职位" prop="majorPosition" width="200" />
...
import { unifyDept, unifyUser } from "@/api/system/unityUser"
export default {
    mixins: [reportMixins],
    props: {
        usenotice: {
    ...
    watch: {
      filterText(val) {
        this.$refs.unifyDeptTree.filter(val);
      }
    },
    mounted() {
        let _this = this
    ...
    methods: {
        getAllDeptTree() {
            ...
            unifyDept({}).then(data => {
                console.log(data)
                _this.unifyDeptTree = data.data.records || [];
            })
            .finally(e => {
        ...
        changeUnifyDept(node, list) {
            ...
            this.$nextTick(() => {
                this.currentKey = arr;
                this.$refs.unifyDeptTree.setCheckedKeys(arr);
                this.startSearch();
            });
        },
        getList(searchData) {
            if (this.currentKey.length > 0) {
                searchData["unifyDeptTree"] = this.currentKey[0];
            }
            // 合并外部使用参数
