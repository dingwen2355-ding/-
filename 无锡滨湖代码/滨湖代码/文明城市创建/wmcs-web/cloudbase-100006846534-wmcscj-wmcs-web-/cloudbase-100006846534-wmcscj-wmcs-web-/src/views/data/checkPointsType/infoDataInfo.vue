<template>
    <baseKanbanInfo :title="useType == 'add' ? '添加点位类型' : ('点位类型 / ' + (item.name || ''))">
...
<script>
import { infoCheckPointsType, addCheckPointsType, editCheckPointsType } from '@/api/data/checkPointsType'
export default {
    props: {
    ...
    data() {
        return {
            ...
            rules: {
                name: [{ required: true, message: '请输入y名称', trigger: ['blur', 'change'] }]
            }
        }
    },
    computed: {
            ...
            this.$refs.ruleForm.validate((valid) => {
                if (valid) {
                    ...
                    .then(d => {
                        ...
                        _this.$store.commit('addSearchFilter', 'getCheckPointsTypeList')
                        _this.refushBack()
                    })
                    .finally(e => {
                        _this.v_loadig = false
