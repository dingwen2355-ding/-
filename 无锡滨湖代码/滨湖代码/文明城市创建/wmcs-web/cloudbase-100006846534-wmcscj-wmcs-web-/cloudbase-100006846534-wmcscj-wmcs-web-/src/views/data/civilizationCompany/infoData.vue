<template>
    <baseKanbanInfo :title="useType == 'add' ? '添加文明单位' : ('文明单位 / ' + (item.name || ''))">
        ...
        <div slot="body" v-loading="v_loadig" class="td-form-content-wap">
            ...
            >
                ...
                <el-row class="show-flex-r wrap" :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="单位类型" prop="selectType">
                            <dirtCivilizationCompanyType
                                ref="civilizationCompanyType"
                                noLabel
                                useType="form"
                                allWidth="100%"
                                :disabled="baseDisable"
                                @change="(v) => { item.selectType = v['civilizationCompanyType'] || '' }"
                            />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                    ...
                    <el-col :span="12">
                        <el-form-item label="社会信用代码" prop="creditCode">
                            <div v-if="!baseDisable">
                                <el-input v-model="item.creditCode" />
                            </div>
                            <div v-else class="td-form-val">
                                {{ item.creditCode || '' }}
                            </div>
                    ...
                    <el-col :span="12">
                        <el-form-item label="平台展示名称" prop="platname">
                            <div v-if="!baseDisable">
                                <el-input v-model="item.platname" />
                            </div>
                            <div v-else class="td-form-val">
                                {{ item.platname || '' }}
                            </div>
                        ...
                        <el-form-item label="宣传图" prop="icon">
                            ...
                            <div class="w-100">
                                <baseUpFile
                                    ...
                                    :showDel="!baseDisable"
                                    onlyImg
                                    tableName="data_civilization_company"
                                    fieldName="icon"
                                />
...
<script>
import { infoCivilizationCompany, addCivilizationCompany, editCivilizationCompany } from '@/api/data/civilizationCompany'
export default {
    props: {
    ...
    methods: {
        ...
        getInfo() {
            let _this = this
            // 获取详情接口
            infoCivilizationCompany(this.id)
            .then(data => {
                let item = data.data
                ...
                _this.$refs.civilizationCompanyType.setValWithChange(item.selectType)
        ...
        editItem() {
            ...
            this.$refs.ruleForm.validate((valid) => {
                if (valid) {
                    if (this.v_loadig) return
                    this.v_loadig = true
                    editCivilizationCompany(upData)
                    .then(d => {
                        _this.$successMessage('操作成功!')
