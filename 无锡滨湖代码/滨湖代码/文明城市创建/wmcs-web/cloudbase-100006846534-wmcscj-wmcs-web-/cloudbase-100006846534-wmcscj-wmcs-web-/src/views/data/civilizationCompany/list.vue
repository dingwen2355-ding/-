        ...
        <baseSearchForm ref="searchTable" @getList="getList" @search="startSearch" @reset="resetQuery">
            <div slot="customQuery" class="listSearch padding-left-10">
                <div class="search-line">
                    ...
                    <normalSearch ref="name" labelName="单位名称" keyName="name" placName="请输入单位名称" />
                    ...
                    <dirtCivilizationCompanyType ref="civilizationCompanyType" labelName="单位类型" keyName="civilizationCompanyType" pplacName="请选择单位类型" />
            ...
            <div slot="table">
                ...
                <div>
                    <el-table
                        ...
                        :data="tableData"
                    >
                        <el-table-column label="单位名称" prop="name" show-overflow-tooltip min-width="250" />
                        ...
                        <el-table-column label="单位类别" prop="selectType" show-overflow-tooltip width="150">
                            <template slot-scope="scope">
...
import { listCivilizationCompany, deleteCivilizationCompany, copyCivilizationCompany } from '@/api/data/civilizationCompany'
export default {
    mixins: [reportMixins],
    data() {
        return {}
    ...
    methods: {
        getList(searchData) {
            this.getTableData(listCivilizationCompany, searchData)
        },
        delItem(item) {
            let _this = this
            new Promise((res, rej) => {
            ...
            .then(d => {
                _this.useDia.hide()
                return deleteCivilizationCompany(item.id)
            })
            .then(d => {
        ...
        copyItemCountry(item) {
            ...
            .then(d => {
                ...
                data.id = item.id;
                data.level = 1;
                return copyCivilizationCompany(data)
            })
            .then(d => {
                _this.$successMessage('操作成功!')
                _this.refushPage()
