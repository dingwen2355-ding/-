<template>
    <el-dialog :title="useType == 'add' ? '添加点位类型' : item.name" :visible.sync="show" append-to-body width="700px" :before-close="cancel">
...
<script>
import { addCheckPointsType, editCheckPointsType } from '@/api/data/checkPointsType'
export default {
    props: {
        show: {
            type: Boolean,
    ...
    data() {
        return {
            ...
            rules: {
                name: [{ required: true, message: '请输入点位类型名称', trigger: ['blur', 'change'] }]
            },
