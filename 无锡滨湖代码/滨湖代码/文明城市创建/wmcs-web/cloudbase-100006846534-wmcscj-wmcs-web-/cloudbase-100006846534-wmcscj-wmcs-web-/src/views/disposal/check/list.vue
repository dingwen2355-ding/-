                        ...
                        <el-table-column label="操作" width="260" fixed="right">
                            <template slot-scope="scope">
                                <div class="show-flex-r a-c">
                                    <div class="zr-link-p unline m-5" @click="toFinish(scope.row)">办结</div>
                                    <div class="zr-link-s unline m-5" @click="refreshGive(scope.row)">回退重派</div>
                                    <div class="zr-link-w unline m-5" @click="refreshTodo(scope.row)">回退重做</div>
                                    <div class="zr-link-d unline m-5" @click="delEvent(scope.row)">工单作废</div>
                                </div>
                            </template>
                        </el-table-column>
                    </el-table>
...
import { check, deleteEvent, refreshGive, refreshTodo } from "@/api/workflow/workflow";
export default {
    mixins: [reportMixins],
    data() {
        return {
    ...
    methods: {
        getList(searchData) {
            this.getTableData(check, searchData)
        },
        toHandle(item) {
            this.$router.push(
                `./_handle?flowinstanceid=${item.flowinstanceid}&taskid=${item.taskid}&id=${item.id}&scope=all`
            );
        },
        toFinish(item) {
            this.$router.push(
        ...
        refreshGive(item) {
            ...
            new Promise((res, rej) => {
                _this.useDia = this.$dialog({
                    header: '系统提示',
                    body: `是否将事件编号"${item.code}"的数据退回重新分派？`,
                    style: 'color: rgba(0, 0, 0, 0.6)',
                    onConfirm: ({ e }) => {
                        res()
                    }
        ...
        refreshTodo(item) {
            ...
            new Promise((res, rej) => {
                _this.useDia = this.$dialog({
                    header: '系统提示',
                    body: `是否将事件编号"${item.code}"的数据退回重新整改？`,
                    style: 'color: rgba(0, 0, 0, 0.6)',
                    onConfirm: ({ e }) => {
                        res()
                    }
