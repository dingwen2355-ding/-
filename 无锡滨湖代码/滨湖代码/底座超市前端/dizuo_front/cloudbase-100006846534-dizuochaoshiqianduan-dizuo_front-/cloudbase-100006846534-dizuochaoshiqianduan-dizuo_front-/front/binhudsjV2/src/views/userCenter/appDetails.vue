        ...
        <div class="appdetails-box">
            <div class="appdetails-box-con block">
                <div class="appdetails-box-top ">
                    ...
                    <div class="userinfo-item-con">
                        ...
                        <div class="item-box-unit">所属部门：
                            {{ fSetSsbm(oAppDetails.dept, aBmData) }}
                        ...
                        <template v-if="oAppDetails.casAppId">
                            ...
                            <el-button type="success" v-else-if="sBtnType === 'applying'">权限申请中</el-button>
                            <el-button type="primary" v-else @click="fCliclSq(oAppDetails)">权限申请</el-button>
                        </template>

                        <template v-else>
                            <el-button type="success" @click="fGoToAdmin(oAppDetails)">系统入口</el-button>
                        </template>
                        <!-- <el-button type="primary" @click="fCliclSq(oAppDetails)">权限申请</el-button> -->
                    </div>
                </div>
                <hr width="90%" />

                <template v-if="oAppDetails.logo">
                    <el-carousel indicator-position="none" :loop="true" :interval="5000" height="700px"
                        class="appdetails-box-image">
                        <el-carousel-item v-for="(item, idx) in oAppDetails.logo.split(',')" :key="'1' + idx">
                            <img :src="item" class="appdetails-box-image" alt="">
                        </el-carousel-item>
                        <el-carousel-item v-for="(item, idx) in oAppDetails.logo.split(',')" :key="'2' + idx">
                            <img :src="item" class="appdetails-box-image" alt="">
                        </el-carousel-item>
                    </el-carousel>
                </template>
                ...
                <div class="file-wrap" v-if="oAppDetails.upload && oAppDetails.upload.length > 0">
                    <dic v-for="(item, idx) in oAppDetails.upload" :key="idx" class="file-item">
...
export default {
    ...
    data() {
        return {
            // sImage: JSON.parse(this.$route.query.data).icon || require('../../assets/image/home/appmarket/szzw_img.png'),
    ...
    mounted() {
        console.log(this.oAppDetails.logo)
        this.fRequestAllpp();
        // console.log(window.location)
        console.log(this.$route.query)
        console.log(this.oAppDetails.upload)
    ...
    methods: {
        ...
        fRequestAuthorityApplyForList(len) {
            ...
            this.$fRequest({
                ...
                if (this.nFlag <= 1) {
                    this.fRequestAuthorityApplyForList(res.data.total);
                }
                // 如果len存在
                if (len) {
                    let aData = res.data.records.filter(item => item.applicationManagementId == this.$route.query.id)
