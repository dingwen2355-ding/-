        ...
        <div class="search-box-wrap">
            <div class="search-box _flexw">
                <template v-if="aRecommend && aRecommend.length > 0">
                    <comList-block :type="sClasses" :data="aRecommend.slice((currentPage4 - 1) * 9, (currentPage4) * 9)"
                        :params="{ ...oParams }" @fCliclDetails="fGoToDetails"></comList-block>
                </template>
                <template v-else>
...
export default {
    ...
    methods: {
        ...
        async fRequestAllpp() {
            ...
            this.aRecommend = this.aAllAppData.filter(item => item.isRecommend == '0');
            this.aRecommendCopy = this.aAllAppData.filter(item => item.isRecommend == '0');
            this.nDataCount = this.aRecommend.length
        },
        // 请求所有的组件/应用/工具
        // fRequestAllpp() {
        //     this.$fRequest({
        ...
        //         this.aAllAppData = aData;// 全部的应用
        //         // 推荐的应用
        //         this.aRecommend = this.aAllAppData.filter(item => item.isRecommend == '1');
        //         this.aRecommendCopy = this.aAllAppData.filter(item => item.isRecommend == '1');
        //         this.nDataCount = this.aRecommend.length
