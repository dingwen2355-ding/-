...
@Component("catalogueManageValidFilesAbstractImpl")
public class CatalogueManageValidFilesAbstractImpl extends AbstractValidFile {
   private static final Logger log = LoggerFactory.getLogger(CatalogueManageValidFilesAbstractImpl.class);
   @Autowired
   ...
   public void validExistData(MultipartFile multipartFile) {
      ...
      try {
         ...
         ((ExcelReaderSheetBuilder) EasyExcel.read(inputStream, ExportSysCatalogueTemplateDTO.class, new ValidExceLegalListener(existData)).sheet().headRowNumber(3)).doRead();
         if (!existData.get()) {
