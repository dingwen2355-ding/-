...
@Component("datasourceValidFilesAbstractImpl")
public class DatasourceValidFilesAbstractImpl extends AbstractValidFile {
   private static final Logger log = LoggerFactory.getLogger(DatasourceValidFilesAbstractImpl.class);
   @Autowired
   ...
   public void validExistData(MultipartFile multipartFile) {
      ...
      try {
         ...
         ((ExcelReaderSheetBuilder) EasyExcel.read(inputStream, ExportDatasourceTemplateDTO.class, new ValidExceLegalListener(existData)).sheet().headRowNumber(1)).doRead();
         if (!existData.get()) {
