...
@Component("sysManageValidFilesAbstractImpl")
public class SysManageValidFilesAbstractImpl extends AbstractValidFile {
   private static final Logger log = LoggerFactory.getLogger(SysManageValidFilesAbstractImpl.class);
   @Autowired
   ...
   public void validExistData(MultipartFile multipartFile) {
      ...
      try {
         ...
         ((ExcelReaderSheetBuilder) EasyExcel.read(inputStream, ExportSysTemplateDTO.class, new ValidExceLegalListener(existData)).sheet().headRowNumber(2)).doRead();
         if (!existData.get()) {
