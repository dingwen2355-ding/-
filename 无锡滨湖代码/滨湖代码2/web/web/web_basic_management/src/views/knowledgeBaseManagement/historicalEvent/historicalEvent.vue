  ...
  <div class="historicalEvent">
    <el-form :inline="true" :model="searchForm" ref="form">
      <el-form-item label="事故名称:">
        <el-input clearable v-model="searchForm.accidentTitle" style="width: 175px;" placeholder="请输入事故名称"></el-input>
      </el-form-item>
      <el-form-item label="事故地点:">
        <el-input clearable v-model="searchForm.accidentAddress" style="width: 175px;" placeholder="请输入事故地点"></el-input>
      </el-form-item>
    ...
    <div class="contentBody">
      ...
      >
        ...
        <el-table-column
          ...
          width="110"
        ></el-table-column>
        <el-table-column align="center" header-align="center" show-overflow-tooltip prop="accidentAddress" label="事故地点"></el-table-column>
    ...
    <AddHistoricalEvent :dialogVisible.sync="showAdd" :formInfo="formInfo" :isCode="isCode" @success="success"></AddHistoricalEvent>
  </div>
</template>
<script>
import request from '@/utils/request'
import AddHistoricalEvent from './option/addHistoricalEvent.vue'
import qs from 'qs'
export default {
  name: 'historicalEvent',
  components: { AddHistoricalEvent },
  data() {
    return {
      tableHeight: 0,
      searchForm: {
  ...
  methods: {
    ...
    queryDataTable() {
      this.loading = true
      let url = '/binhuapis/HistoricalEvent/queryHistoricalEventList?pageNum=' + this.currentPage + '&pageSize=' + this.pageSize
      if (this.searchForm.accidentTitle) {
        url = url + '&accidentTitle=' + this.searchForm.accidentTitle
      }
      if (this.searchForm.accidentAddress) {
        url = url + '&accidentAddress=' + this.searchForm.accidentAddress
      }
      if (this.searchForm.accidentType) {
        url = url + '&accidentType=' + this.searchForm.accidentType
      }
    ...
    deleteData() {
      if (this.multipleSelection.length > 0) {
        this.$confirm('此操作将删除所勾选的事件, 是否继续?', '提示', {
          ...
          let url = `/binhuapis/HistoricalEvent/deleteHistoricalEvent?ids=${ids}`
    ...
    // 删除
    handleDelete(row) {
      this.$confirm('此操作将删除所选的事件, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        let url = '/binhuapis/HistoricalEvent/deleteHistoricalEvent?ids=' + row.id
        // console.log('删除--url', url)
