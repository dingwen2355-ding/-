<template>
  <div class="standardSpecification">
    <el-form :inline="true" :model="searchForm" ref="form">
      <el-form-item label="文件名:">
        <el-input clearable v-model="searchForm.fileName" style="width: 175px;" placeholder="请输入文件名称"></el-input>
      </el-form-item>
      <el-form-item label="单位:">
        <el-input clearable v-model="searchForm.preparationUnit" style="width: 175px;" placeholder="请输入编制单位"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <div class="contentBody">
      ...
      >
        ...
        <el-table-column
          ...
          label="安全管理类型"
        ></el-table-column>
        <el-table-column align="center" header-align="center" show-overflow-tooltip prop="preparationUnit" label="编制单位"></el-table-column>
        <el-table-column
          align="center"
          header-align="center"
          show-overflow-tooltip
    ...
    <AddStandardSpecification :dialogVisible.sync="showAdd" :formInfo="formInfo" :isCode="isCode" @success="success"></AddStandardSpecification>
  </div>
</template>
<script>
import request from '@/utils/request'
import AddStandardSpecification from './option/addStandardSpecification.vue'
import qs from 'qs'
export default {
  name: 'standardSpecification',
  components: { AddStandardSpecification },
  data() {
    return {
      tableHeight: 0,
      searchForm: {
  ...
  methods: {
    ...
    queryDataTable() {
      this.loading = true
      let url = '/binhuapis/Specification/querySpecificationList?pageNum=' + this.currentPage + '&pageSize=' + this.pageSize
      if (this.searchForm.fileName) {
        url = url + '&fileName=' + this.searchForm.fileName
      }
      if (this.searchForm.preparationUnit) {
        url = url + '&preparationUnit=' + this.searchForm.preparationUnit
      }
    ...
    deleteData() {
      if (this.multipleSelection.length > 0) {
        this.$confirm('此操作将删除所勾选的标准规范, 是否继续?', '提示', {
          ...
          let url = `/binhuapis/Specification/deleteByPrimaryKey?ids=${ids}`
    ...
    // 删除
    handleDelete(row) {
      this.$confirm('此操作将删除所选的标准规范, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        let url = '/binhuapis/Specification/deleteByPrimaryKey?ids=' + row.id
        // console.log('删除--url', url)
