        ...
        >
          ...
          <el-form-item label="附件">
            <div style="display: flex; align-items: center; margin-top: 20px">
              ...
              />
              <!-- </div> -->
              <!-- <div class="event-image__preview" v-if="form.filePicture"> -->
              <el-image
          ...
          <div class="title-bar" style="margin: 16px 0">
            <div class="bar-icon"></div>
            <p class="bar-text">核查要求</p>
          </div>
          <el-form-item label="核查人" required>
            <el-input clearable v-model="verifyInfo.checkPersonName" placeholder="请输入核查人"></el-input>
          </el-form-item>
          <el-form-item label="下发时间" prop="issueTime">
            <el-date-picker
              ...
              value-format="yyyy-MM-dd HH:mm:ss"
              key="sendTime"
              v-model="verifyInfo.issueTime"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="下发要求" required>
            <el-input
              clearable
              v-model="verifyInfo.requirement"
              placeholder="请输入下发要求"
              style="width: 850px"
            ></el-input>
          </el-form-item>
          <div class="title-bar" style="margin: 16px 0">
            <div class="bar-icon"></div>
            <p class="bar-text">核查反馈</p>
          </div>
          <el-form-item label="是否属实" required>
            <el-input clearable v-model="feedbackInfo.isTrue" placeholder="请选择是否属实"></el-input>
          </el-form-item>
          <el-form-item label="核查时间" prop="checkTime" required>
            <el-date-picker
              type="datetime"
              placeholder="选择核查时间"
              format="yyyy-MM-dd HH:mm:ss"
              value-format="yyyy-MM-dd HH:mm:ss"
              key="verifyTime"
              v-model="feedbackInfo.checkTime"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="核查内容" required>
            <el-input
              clearable
              v-model="feedbackInfo.checkContent"
              placeholder="请输入核查内容"
              style="width: 850px"
            ></el-input>
          </el-form-item>
          <el-form-item label="核查附件">
            <div style="display: flex; align-items: center; margin-top: 20px">
              <video
                ...
                style="width: 210px; height: 140px; margin-right: 16px"
                controls
                v-if="feedbackInfo.fileUrls"
                :src="feedbackInfo.fileUrls"
              />
              <div v-if="isCode !== 1" class="upload_box">
                <input
                  ref="videoFile"
...
export default {
  ...
  watch: {
    dialogVisible(v) {
      ...
      if (v) {
        ...
        this.queryVerifyInfo(this.formInfo.id)
    ...
    'verifyInfo.id'(val) {
      // 核查反馈
      let url = '/binhuapis/eventCheck/query-feedback'
      url += '?checkId=' + val
      request.get(url).then((res) => {
        if (res.data.code === 200) {
          let data = res.data.data.list || []
          this.feedbackInfo = data[0] || {}
        }
      })
    }
  },
  ...
  computed: {
    eventFileVoices() {
      if (!this.form.fileVoice) return []
      return this.form.fileVoice.split(',')
    },
    eventImages() {
      if (!this.form.filePicture) return []
      return this.form.filePicture.split(',')
    },
    sourceListSort() {
  ...
  methods: {
    ...
    fileUpload(v) {
      ...
      request.post(url, formData).then((res) => {
        if (res.data.code === 200) {
          this.form.fileVideo = res.data.data
