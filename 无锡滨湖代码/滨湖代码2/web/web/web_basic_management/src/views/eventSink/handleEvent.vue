  ...
  <div class="drillEventEntry">
    ...
    <div class="contentBody">
      ...
      >
        ...
        <el-table-column prop="status" label="是否核查">
          <template slot-scope="scope">
            <span>{{ scope.row.status === 40 ? '否' : '是' }}</span>
          </template>
        </el-table-column>
...
import addDialog from './option/acceptEventDialog.vue'
  ...
  methods: {
    ...
    queryDictDictTree() {
      ...
      request.get(url).then((res) => {
        if (res.data.code === 200) {
          let data = res.data.data || []
          let eventStatusList = data
            .filter((item) => item.dictName === '待核查' || item.dictName === '已核查')
            .map((item) => item.id)
    ...
    queryDataTable() {
      this.loading = true
      let url =
        '/binhuapis/dapeng/event/query-condition?statusIds=' +
        this.statusIds +
        '&page=' +
        this.currentPage +
        '&pageSize=' +
    ...
    handleEdit(row, code) {
      ...
      this.verifyInfo.eventId = row.id
      this.formInfo = row
    },
