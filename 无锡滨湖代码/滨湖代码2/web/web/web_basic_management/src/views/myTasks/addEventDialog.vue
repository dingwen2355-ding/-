          ...
          <el-form-item label="处置附件">
            <div style="display: flex; align-items: center; margin-top: 20px">
              <div class="upload_box">
                ...
                <div v-show="isCode === 1">{{ videoFileName }}</div>
              </div>
              <div class="upload_box">
                <input
                  :disabled="isCode === 1"
                ...
                <div v-show="isCode === 2">
                  ...
                  <el-progress
                    ...
                    :text-inside="false"
                    :status="uploadPictureProgress === 100 ? 'success' : ''"
                    :stroke-width="10"
                  />
                </div>
              ...
              <div class="upload_box">
                <input
                  ...
                  style="font-size: 14px; margin-left: 20px"
                  id="video_file"
                  accept=".xlsx, .xls, .doc, .docx, .pdf, .PDF, .pptx"
                />
                <div v-show="isCode === 2">
                  <i class="el-icon-document-copy" style="font-size: 26px; color: #0052d9"></i>
                  <p style="color: #0052d9">上传文件</p>
                  <!-- <p style="color: #5b6472">上传的文件格式为mp3/wav</p> -->
                  <el-progress
                    v-show="uploadAudioProgress !== 0"
                    :percentage="uploadAudioProgress"
                    :text-inside="false"
                    :status="uploadAudioProgress === 100 ? 'success' : ''"
                    :stroke-width="10"
                  ></el-progress>
                </div>
                <div v-show="isCode === 1">{{ audioFileName }}</div>
              </div>
            </div>
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    audioChange(e) {
      ...
      files.forEach((item) => {
        ...
        //.xlsx｜xls｜doc｜docx｜pdf｜PDF｜pptx
        if (!/\.(xlsx|xls|doc|docx|pdf|PDF|pptx)$/.test(name)) {
    ...
    reset() {
      ...
      this.files = null
      this.form = {}
      this.uploadPictureProgress = this.uploadProgress = this.uploadAudioProgress = 0
    ...
    submitAll() {
      ...
      url = '/binhuapis/dapeng/eventTask/edit?userId=' + sessionStorage.getItem("userId") + '&userName=' + sessionStorage.getItem("userName")
