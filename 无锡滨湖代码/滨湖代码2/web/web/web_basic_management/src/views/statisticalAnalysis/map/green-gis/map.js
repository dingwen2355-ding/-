...
export class Map extends Subject {
    ...
    constructor(id, option) {
        ...
        //mousemove: 鼠标移动时
        //resize: 视图容器尺寸调整时
        super(["extent", "click", "dblclick", "mousemove", "resize", 'wheel', 'mouseup']);
        this._option = {
    ...
    showText(feature, text, offset) {
        const center = feature.geometry.getCenter(CoordinateType.Projection, this.projection);
        const matrix = this._ctx.getTransform();
        const screenX = (matrix.a * center[0] + matrix.e) + offset.x;
        const screenY = (matrix.d * center[1] + matrix.f) + offset.y;
        this._tooltip.show(text, screenX, screenY);
    }
    /**
     * 隐藏Tooltip
