        ...
        >
          ...
          <el-form-item label="上传文件">
            <div style="display: flex; align-items: center; margin-top: 20px">
              <!--              <div v-if="videoShow" style="position: relative; width: 210px; height: 140px; margin-right: 16px">-->
              <!--                <video class="video" style="width: 210px; height: 140px; display: block" controls :src="videoShow" />-->
              ...
              <!-- <div v-show="isCode === 1 && form.fileVideo">
                {{ videoFileName }}
              </div>-->
              <div class="upload_box" v-show="isCode === 2">
                <input
              ...
              <!-- <div v-show="isCode === 1" v-if="form.filePicture">
                {{ pictureFileName }}
              </div>-->
              <div class="upload_box" v-show="isCode === 2">
                <input
          ...
          <!-- <div style="padding-left: 40px; margin: 16px 0">
            <i class="el-icon-document" style="color: #0052d9; font-size: 16px; font-weight: bold">核查要求</i>
          </div>
          <el-form-item label="核查人">
            <el-input clearable v-model="form.monitorUnit" placeholder="请输入监督单位"></el-input>
          </el-form-item>
          <el-form-item label="下发时间" prop="monitorUnit">
            <el-date-picker
              type="datetime"
              placeholder="选择时间"
              format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-form-item label="下发要求">
            <el-input clearable v-model="form.monitorUnit" placeholder="请输入监督单位" style="width: 400px"></el-input>
          </el-form-item>
          <div style="padding-left: 40px; margin: 16px 0">
            <i class="el-icon-document" style="color: #0052d9; font-size: 16px; font-weight: bold">核查反馈</i>
          </div>
          <el-form-item label="是否属实">
            <el-input clearable v-model="form.monitorUnit" placeholder="请输入监督单位"></el-input>
          </el-form-item>
          <el-form-item label="核查时间" prop="monitorUnit">
            <el-date-picker
              type="datetime"
              placeholder="选择时间"
              format="yyyy-MM-dd HH:mm:ss"
...
export default {
  ...
  computed: {
    ...
    filterCommunityList() {
      if (this.streetList.length === 0) return []
      if (!this.formInfo.streetName) return this.communityList
      let filterObjs = this.streetList.filter((item) => {
        return this.formInfo.streetName === item.precinctName
      })
      let precinctId = ''
      if (filterObjs.length > 0) {
        precinctId = filterObjs[0].precinctId
        return this.communityList.filter((item) => item.up_precinct_id === precinctId)
      } else {
  ...
  methods: {
    ...
    changeStreet(val) {
      if (val) {
        ...
        //   }
        // })
        const filterCommunity = this.filterCommunityList.filter((item) => item.precinctName === val)
