<template>
  <div class="planResponse">
    <el-form :inline="true" :model="searchForm" ref="form">
      ...
      <el-form-item>
        <el-select v-model="searchForm.typeName" style="width: 220px" placeholder="请选择事故类型" clearable>
    ...
    <div class="contentBody">
      ...
      >
        ...
        <el-table-column prop="eventTitle" label="事故标题"></el-table-column>
        <el-table-column prop="communityName" label="所在社区"></el-table-column>
        <el-table-column prop="streetName" label="所在街道"></el-table-column>
        ...
        <el-table-column label="操作" width="380">
          <template slot-scope="scope">
            ...
            >详情</el-button>-->
            <el-button
              v-if="scope.row.eventPlanId"
              @click="handleShowVideo(scope.row, 0)"
              type="primary"
              plain
              size="mini"
              >视频</el-button
            >
            <el-button
              v-if="[41].includes(scope.row.status) && scope.row.eventPlanId"
            ...
            >
            <el-button
              v-if="[42].includes(scope.row.status) && scope.row.eventPlanId"
              @click="handleFinish(scope.row, 1)"
              type="success"
              plain
              size="mini"
              >完结</el-button
            >
            <!-- <el-button v-if="[43].includes(scope.row.status) && scope.row.eventPlanId" type="success" plain size="mini"
              >已完结</el-button> -->
            <!-- v-if="![41, 42].includes(scope.row.status)" -->
            <el-button @click="handleEdit(scope.row, 0)" type="success" plain size="mini">编辑</el-button>
            <!-- v-if="![41, 42].includes(scope.row.status)" -->
    ...
    <edit-plan-dialog :dialogVisible.sync="editPlanVisible" :isCode="isCode" :formInfo="formInfo"></edit-plan-dialog>
    <video-monitoring :dialogVisible.sync="videoVisible" :eventId="formInfo.id"></video-monitoring>
    <QueryUserDialog :dialogVisibleUser.sync="showConfigOrganization" :dataInfo="leaderArr" :formInfo="formInfo"></QueryUserDialog>
  </div>
</template>
<script>
import addDialog from './option/addDialog.vue'
import request from '@/utils/request'
import editPlanDialog from '@/views/PlanResponse/option/editPlanDialog'
import videoMonitoring from '@/views/PlanResponse/option/videoMonitoring'
import QueryUserDialog from './option/QueryUserDialog'
export default {
  components: { addDialog, editPlanDialog, videoMonitoring, QueryUserDialog },
  name: 'planResponse',
  data() {
    return {
      tableHeight: 0,
      ...
      streetList: [],
      statusIds: '',
      showConfigOrganization: false,
      leaderArr: []
    }
  },
  mounted() {
    this.queryDictDictTree()
    ...
    this.$root.eventBus.$on('configOrganization', (data) => {
      this.leaderArr = data
      this.showConfigOrganization = true
    })
  },
  watch: {
    statusIds() {
  ...
  methods: {
    // precinctKind： 110-街道 120-社区 upPrecinctId： 上级id
    getPrecinctList() {
    ...
    queryDictDictTree() {
      ...
      request.get(url).then((res) => {
        if (res.data.code === 200) {
          let data = res.data.data || []
          let eventStatusList = data.filter((item) => item.dictName !== '待核查').map((item) => item.id)
          this.statusIds = eventStatusList.join(',')
        }
      })
    },
    queryDataTable() {
      this.loading = true
      let url =
        '/binhuapis/dapeng/event/query-condition?statusIds=41,42,43,135' +
        '&page=' +
        this.currentPage +
        '&pageSize=' +
        this.pageSize
    ...
    handleEdit(row, code) {
      ...
      this.formInfo = row
      this.editPlanVisible = !this.editPlanVisible
    },
    handleShowVideo(row, code) {
      this.formInfo = row
      this.videoVisible = true
    },
    handleStatus(row, status) {
      let url = '/binhuapis/dapeng/event/edit'
      request
        .post(url, {
          id: row.id,
          status: status
    ...
    handleStart(row, code) {
      this.$confirm('此操作将启动预案, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          this.handleStatus(row, '42')
        })
        .catch(() => {
          this.$message({
            type: 'info',
    ...
    handleFinish(row, code) {
      this.$confirm('此操作将完成预案, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          this.handleStatus(row, '43')
        })
        .catch(() => {
          this.$message({
            type: 'info',
