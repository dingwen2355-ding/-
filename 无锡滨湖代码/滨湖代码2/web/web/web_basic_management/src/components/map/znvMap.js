...
ZnvMap.showLayerByIds = function (ids) {
  let markers = []
  ids.forEach(e => {
    markers.push(layerMarks[e])
  })
  ZnvMap.showMarkers(markers)
  return markers
}
...
// 根据过滤条件展示点位
ZnvMap.showPointByKey = function (filterList, type = 'or') {
  let showMarkers = []
  let markerList = []
  let markerIdList = []
  for (let key in layerMarks) {
  ...
  if (type === 'and') {
    markerList.forEach((m) => {
      ...
      if (flag && !markerIdList.includes(id)) {
        showMarkers.push(m)
        markerIdList.push(id)
      }
    })
  } else {
