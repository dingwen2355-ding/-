<template>
  <div class="VideoInspectionModule">
      ...
      <div class="video-item">
        <div class="select-video">
          <el-select v-model="value1" placeholder="视频类型选择" style="width: 100%" @change="changeFullType(1, $event)">
            <el-option v-for="item in videoTemp" :key="item.deviceKind" :label="item.name" :value="item.deviceKind"> </el-option>
          </el-select>
        </div>
        <div class="set-video">
          <div class="icon-video">
            ...
            <div class="video-btn full-btn">
              <!--              {{full1===''?'选择视频':full1}}-->
              <div class="full-btn1" @click="handleFull(1, 1)" :title="full1.deviceName" v-if="value1 === 666">{{ full1.deviceName ? full1.deviceName : '选择视频' }}</div>
              <div class="full-btn1" @click="handleFull(1, 2)" :title="full1.deviceName" v-else-if="value1 === 31">{{ full1.deviceName ? full1.deviceName : '选择视频' }}</div>
              <el-select v-else class="video-btn full-btn" v-model="full1" filterable placeholder="选择视频">
                <el-option class="excend" v-for="item in fullList1" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
              </el-select>
            </div>
          </div>
        </div>
        ...
        <div class="set-video">
          <div class="icon-video">
            ...
            <div class="video-btn full-btn">
              ...
              <div class="full-btn1" @click="handleFull(2, 1)" :title="full2.deviceName" v-if="value2 === 666">{{ full2.deviceName ? full2.deviceName : '选择视频' }}</div>
              <div class="full-btn1" @click="handleFull(2, 2)" :title="full2.deviceName" v-else-if="value2 === 31">{{ full2.deviceName ? full2.deviceName : '选择视频' }}</div>
              <el-select v-else class="video-btn full-btn" v-model="full2" filterable placeholder="选择视频">
                <el-option class="excend" v-for="item in fullList2" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
              </el-select>
            </div>
          </div>
        </div>
          ...
          <div class="set-video-small">
            <div class="icon-video">
              ...
              <div class="video-btn full-btn">
                ...
                <div class="full-btn1" @click="handleFull(3, 1)" :title="full3.deviceName" v-if="value3 === 666">{{ full3.deviceName ? full3.deviceName : '选择视频' }}</div>
              <div class="full-btn1" @click="handleFull(3, 2)" :title="full3.deviceName" v-else-if="value3 === 31">{{ full3.deviceName ? full3.deviceName : '选择视频' }}</div>
                <el-select v-else class="video-btn full-btn" v-model="full3" filterable placeholder="选择视频">
                <el-option class="excend" v-for="item in fullList3" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
              </el-select>
              </div>
            </div>
          </div>
          ...
          <div class="set-video-small">
            <div class="icon-video">
              ...
              <div class="video-btn full-btn">
                ...
                <div class="full-btn1" @click="handleFull(4, 1)" :title="full4.deviceName" v-if="value4 === 666">{{ full4.deviceName ? full4.deviceName : '选择视频' }}</div>
                <div class="full-btn1" @click="handleFull(4, 2)" :title="full4.deviceName" v-if="value4 === 31">{{ full4.deviceName ? full4.deviceName : '选择视频' }}</div>
                <el-select v-else class="video-btn full-btn" v-model="full4" filterable placeholder="选择视频">
                <el-option class="excend" v-for="item in fullList4" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
              </el-select>
              </div>
            </div>
          </div>
          ...
          <div class="set-video-small">
            <div class="icon-video">
              ...
              <div class="video-btn full-btn">
                ...
                <div class="full-btn1" @click="handleFull(5, 1)" :title="full5.deviceName" v-if="value5 === 666">{{ full5.deviceName ? full5.deviceName : '选择视频' }}</div>
                <div class="full-btn1" @click="handleFull(5, 2)" :title="full5.deviceName" v-else-if="value5 === 31">{{ full5.deviceName ? full5.deviceName : '选择视频' }}</div>
                <el-select v-else class="video-btn full-btn" v-model="full5" filterable placeholder="选择视频">
                  <el-option class="excend" v-for="item in fullList5" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
                </el-select>
              </div>
            </div>
          </div>
          ...
          <div class="set-video-small">
            <div class="icon-video">
              ...
              <div class="video-btn full-btn">
                ...
                <div class="full-btn1" @click="handleFull(6, 1)" :title="full6.deviceName" v-if="value6 === 666">{{ full6.deviceName ? full6.deviceName : '选择视频' }}</div>
                <div class="full-btn1" @click="handleFull(6, 2)" :title="full6.deviceName" v-else-if="value6 === 31">{{ full6.deviceName ? full6.deviceName : '选择视频' }}</div>
                <el-select v-else class="video-btn full-btn" v-model="full6" filterable placeholder="选择视频">
                  <el-option class="excend" v-for="item in fullList6" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"> </el-option>
                </el-select>
              </div>
            </div>
          </div>
    ...
    <div class="select-content" v-if="currentTab === '2'">
      ...
      <div class="select-item">
        <el-input v-model="name" placeholder="请输入群聊名称" clearable autofocus="autofocus"> </el-input>
      </div>
    </div>
    <div class="vx-content" v-if="currentTab === '2'">
      <div class="vx-content-item">
        <div class="vx-title"><i class="iconfont iconzhgl_title"></i><span>部门</span></div>
        <div class="vx-list">
          <ul>
            <li v-for="(value, key, index) in chatAll" @click="changeDept(value, key, index)" :class="{ active: deptChecked === index }" :key="key" :title="key">
              <i class="iconfont iconzhihui_tongxunlu_bumen"></i> {{ key }}
            </li>
          </ul>
        </div>
      </div>
      <div class="vx-content-item-1" v-if="itemOne">
        <el-transfer
          ...
          :data="peopleList"
        >
          <span slot-scope="{ option }" class="people-list" :class="{ isNone: deptType !== '' && option.department !== deptType }">{{ option.name }}{{ option.account }}</span>
        </el-transfer>
      </div>
    </div>
    ...
    <div class="addDialog" v-show="addDialogShow">
      <div class="add-dialog-content">
        ...
        <div class="leftForm">
          <div class="topTitle">可选列表<span style="float: right;">共 {{ leftForm.count }} 条</span></div>
          <div style="display:flex;margin-top:5px">
          <!-- <el-select v-if="!precinctId" clearable v-model="valueOptions" placeholder="请选择区域">
            <el-option
            ...
            <el-input
              placeholder="请输入搜索内容"
              v-model="leftForm.search">
            </el-input>
            <el-button style="font-size: 30px;" icon="el-icon-search" @click="leftSearch"></el-button>
          </div>
          <div class="mainContent">
            <el-checkbox-group v-model="leftForm.chooseList">
              <el-checkbox class="leftItem" v-for="(item,index) in addList" :key="index" :label="item" :title="item.device_name">
                {{item.device_name}}
              </el-checkbox>
            </el-checkbox-group>
          </div>
        ...
        <div class="leftForm">
          ...
          <div style="display:flex;margin-top:5px">
            ...
            <el-button style="font-size: 30px;" icon="el-icon-search" @click="rightSearch"></el-button>
          </div>
          <div class="mainContent">
    ...
    <div class="addDialog" v-show="addDialogRight">
      <div class="add-dialog-content" style="width: 1200px">
        ...
        <div style="display:flex">
        <div class="leftForm">
          <div class="topTitle">可选列表<span style="float: right;">共 {{ allForm.count }} 条</span></div>
          <div style="display:flex;margin-top:5px">
            <!-- <el-select v-if="!precinctId" clearable v-model="valueOptions1" placeholder="请选择区域">
            <el-option
              v-for="item in options"
              :key="item.id"
              :label="item.name"
    ...
    <div class="addDialog" v-show="callingNumberSettingShow">
      <div class="add-dialog-content">
        ...
        <div class="transfer-content">
          <el-transfer
            v-model="callingNumberList"
            :filterable="true"
            :data="videoListAll.mobileIndividual"
            @change="changeTransfer"
            @left-check-change="getLeftChangeCallingNumber"
            :left-default-checked="leftCheckedKeys"
            @right-check-change="getRightChangeCallingNumber"
...
export default {
  ...
  watch: {
    ...
    valueOptions1: {
      handler(v) {
        if (this.showType === 1) {
          this.queryFullAll(this.showRight, 666)
        } else {
          this.queryFullAll(this.showRight, 31)
        }
      }
    },
    callingNumber: {
  ...
  methods: {
    ...
    showAdd(type) {
      if (this.currentTab === '0') {
        ...
        this.addList = this.videoListAll['consultationVideo'].concat(this.videoListAll['mobileIndividual']).concat(this.videoListAll['fixedVideo'])
    ...
    submitAll() {
      if (this.currentTab === '0') {
        ...
        let deviceIds = (this.full1.deviceId ? this.full1.deviceId : this.full1) + ',' +
    ...
    queryAll() {
      ...
      if (this.valueOptions) {
        url += '&precinctId=' + this.valueOptions
      }
      Axios.get(url).then((res) => {
    ...
    queryFullAll(numType, event) {
      ...
      if (event) {
        if (event !== 666) {
          if (event === 31) {
            url += '?deviceKind=' + event + '&devtype=' + '威尔信' + '&pageNum=' + this.allForm.currentPage + '&pageSize=' + this.allForm.pageLen
          } else {
        ...
        if (this.valueOptions1) url += '&precinctId=' + this.valueOptions1
      }
      Axios.get(url).then((res) => {
    ...
    getLeftChangeCallingNumber(val, val1) {
      ...
      if (this.leftCheckedKeys.length > 1 && this.callingNumberList.length === 0) {
        ...
        // eslint-disable-next-line eqeqeq
      } else if (this.callingNumberList.length > 0) {
        // 穿梭框右侧有值的时候，左侧一直禁用
        this.videoListAll.mobileIndividual.map((item, index) => {
          // eslint-disable-next-line eqeqeq
