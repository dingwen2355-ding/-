<template>
  <div class="linkMeeting">
    ...
    <k-dialog
      :dialog-title="showIndex === '1' ? '新增预案': '编辑预案'"
...
export default {
  ...
  mounted() {
    this.$root.eventBus.$on('linkmeeting', (data) => {
      this.value = data
  ...
  methods: {
    ...
    onSubmit(form) {
      ...
      // this.superNameOptionAll.forEach(item => {
      //   this.form.uri.split(',').forEach(res => {
      //     if (res === item.name) {
      //       setting.push(item.uri)
      //     }
      //   })
      // })
