<template>
  <div class="Personment">
    ...
    <k-dialog
      :dialog-title="showIndex === '1' ? '导入人员': '编辑人员'"
          ...
          <div class="form-container" v-else>
            ...
            <el-form-item label="座机">
              ...
              <el-form-item label="所属部门">
                <el-cascader placeholder="请选择所属部门" v-model="form.emergencyPlanId" :options="superNameOption" :props="{ value: 'id', label: 'name', checkStrictly: true }" clearable>
...
export default {
  ...
  watch: {
    'form.emergencyPlanId': {
      handler(v) {
        if (v) {
          this.tableData.forEach(item => {
            if (v === item.id) {
              this.form.emergyPerson = item.name
            }
          })
        }
        console.log(this.form.emergyPerson, '233333333')
      }
    },
    value: {
      handler(v) {
  ...
  methods: {
    ...
    upSubmit() {
      if (this.newList.length > 0) {
        let url = '/apis/communication/uploadPeopleExcel'
    ...
    delClick(row) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        let url = '/apis/communication/deletePeopleById?ids=' + row.id
    ...
    getEmergencyPlanOptions() {
      let url = '/apis/communication/queryPeopleList'
      if (this.search) {
    ...
    showAddRiskDialog(index, row) {
      ...
      if (this.showIndex === '1') {
        ...
        this.form.emergencyPlanId = row.unitId
        this.form.emergyPerson = row.unitName
        this.form.name = row.name
        this.form.id = row.id
        this.form.phone = row.phone
        this.form.position = row.duty
        this.form.tel = row.landLine
      }
      this.dialogVisible = true
    },
    onSubmit(form) {
      if (this.form.emergencyPlanId.length === 0 || !this.form.emergencyPlanId) {
      ...
      let url = '/apis/communication/savePeople'
      let people = {
        name: this.form.name,
        phone: this.form.phone,
        unitId: Array.isArray(this.form.emergencyPlanId) ? this.form.emergencyPlanId.pop() : this.form.emergencyPlanId,
        duty: this.form.position,
        landLine: this.form.tel
      }
      if (this.form.id) {
        people.id = this.form.id
      }
      Axios.post(url, people).then((res) => {
