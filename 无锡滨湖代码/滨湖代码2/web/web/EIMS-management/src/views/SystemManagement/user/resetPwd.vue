...
export default {
  ...
  data() {
    ...
    return {
      dialogCenter: false,
      pform: { oldpassword: '', password: '', checkPass: '' }, // 重置密码
      pwdrules: {
        oldpassword: [{ required: true, message: '请再次输入旧密码', trigger: 'blur' }],
        password: [
  ...
  watch: {
    dialogPasswordVisible(v) {
      if (v) {
        this.pform = { oldpassword: '', password: '', checkPass: '' }
      }
    }
  },
  methods: {
    savePwd() {
      this.$refs.pform.validate((valid) => {
        if (valid) {
          let url = `/gtw/EIMS-management/user/changeSelfPassword?oldPassword=${Encrypt(
            this.pform.oldpassword
          )}&newPassword=${Encrypt(this.pform.password)}`
