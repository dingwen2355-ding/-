...
export default {
  ...
  methods: {
    ...
    getStockNameList() {
      let url = `/gtw/EIMS-management/stockInfoData/getStuff?userLevel=${localStorage.level}`
      axios.get(url).then((r) => {
        ...
        this.stockList = arr
        if (arr.length > 0) {
          this.form.storageName = arr[0].value
          this.form.stockId = arr[0].id
        }
      })
    },
    getGoodNameList() {
    ...
    handleOk() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          // console.log(this.form.operationNum)
          if (this.form.operationNum == 0) {
            this.$message('入库数量不能为0')
            return
          }
          if (this.form.stockId === '') {
          ...
          let url = `/gtw/EIMS-management/stockInfoData/stuffInsert?stockId=${this.form.stockId}&userLevel=${
            localStorage.level
          }&stuffId=${this.form.goodId}&brand=${this.form.brand === '' ? '无' : this.form.brand}&model=${
            this.form.goodInfo === '' ? '无' : this.form.goodInfo
          }&stuffNumber=${this.form.operationNum}&manager=${this.form.manager}&recipientTime=${
            this.form.recipientTime
          }&source=${this.form.source}&identifier=${this.form.identifier}`
          axios.get(url).then((r) => {
            // console.log(r)
            if (r.data.code === 200) {
