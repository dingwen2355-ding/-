      ...
      <div class="info_body" v-loading="loading">
        ...
        >
          <el-form-item label="名称" prop="name">
            <el-input clearable v-model="form.name" placeholder="请输入预案名称" style="width: 630px"></el-input>
          </el-form-item>
          ...
          <el-form-item label="上传文档" prop="fileUrl">
            <el-upload
              ...
              :auto-upload="false"
              accept=".doc,.docx,.pdf,.PDF,.xlc,.jpg,.PNG,.JPGF"
            >
              <el-button slot="trigger" size="small" type="primary">上传文档</el-button>
              <div slot="tip" class="el-upload__tip">只能上传word/pdf/excel/jpg/PNG/JPGF文件，且不超过50MB</div>
            </el-upload>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    submitAll() {
      ...
      this.newFormD.planNumDataBeans = []
      let aboutData = {}
      let url = ''
      if (this.formInfo && this.formInfo.id) {
        url = '/gtw/EIMS-management/update-plan-num'
        for (let item of this.fileList) {
          ...
          this.newFormD.planNumDataBeans.push(aboutData)
        }
      } else {
        url = '/gtw/EIMS-management/add-plan-num'
        for (let item of this.fileList) {
          // console.log('item', item)
          aboutData = {
            fileUrl: item.url,
