      ...
      <div class="info_body" v-loading="loading">
        ...
        >
          ...
          <el-form-item label="级别" prop="levelId" v-if="addShow">
            <el-select filterable clearable v-model="form.levelId" placeholder="请选择预案级别" style="width: 100%">
          ...
          <el-form-item label="分类" prop="eventTypeId" v-if="addShow">
            <el-select filterable clearable v-model="form.eventTypeId" placeholder="请选择分类" style="width: 100%">
              <el-option
  ...
  methods: {
    ...
    submitAll() {
      ...
      if (this.formInfo && this.formInfo.id) {
        url = '/gtw/EIMS-management/update-plan'
        for (let item of this.fileList) {
        ...
        url = '/gtw/EIMS-management/add-plan'
        for (let item of this.fileList) {
          // console.log('item', item)
          aboutData = {
            fileUrl: item.url,
