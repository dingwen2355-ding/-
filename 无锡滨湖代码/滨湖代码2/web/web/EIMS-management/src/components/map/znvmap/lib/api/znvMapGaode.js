...
function gaodeMap(aMapUIMap) {
  ...
  var optsD = {
    ...
    cursor: 'pointer',
    style: [{
        url: require('../../../../../assets/mass0.png'),
        anchor: new AMap.Pixel(6, 6),
        size: new AMap.Size(45, 45)
      }, {
        url: require('../../../../../assets/mass2.png'),
  ...
  gaodeMap.MassMarks = function(map, data, opts) {
    let massMarks = new AMap.MassMarks(data, opts || optsD)
  ...
  gaodeMap.MapLib.MouseTool.prototype = {
    ...
    // 画框
    _selectCircleCompareLngLat: function(pns, cb, opts) {
      let map = this.map
      let mouseTool = this.mouseTool
      let rectOptions = {
        strokeColor: '#F6A21C',
      ...
      map.plugin(['AMap.MouseTool'], function() {
        ...
        AMap.event.addListenerOnce(mouseTool, 'draw', function callback(e) {
          ...
          for (let i = 0; i < pns.length; i++) {
            let pt = new AMap.LngLat(pns[i].lng, pns[i].lat)
            if (obj.contains(pt)) {
              rePns.push(pns[i])
            }
          }
    ...
    // 框选
    _selectRectangleCompareLngLat: function(pns, cb, opts) {
      let map = this.map
      let mouseTool = this.mouseTool
      let me = this
      let rectOptions = {
    ...
    // 多边形选
    _selectPolygonCompareLngLat: function(pns, cb, opts) {
      let map = this.map
      let mouseTool = this.mouseTool
      let me = this
      let polygonOptions = {
      ...
      if (opts) {
        polygonOptions = $.extend({}, polygonOptions, opts)
      }
      map.plugin(['AMap.MouseTool'], function() {
        if (!mouseTool) {
          mouseTool = new AMap.MouseTool(map)
  ...
  gaodeMap.ImageLayer = function(opts) {
    var imageLayer = new AMap.ImageLayer({
      url: opts.imageUrl,
      bounds: new AMap.Bounds([opts.bounds[0], opts.bounds[1]], [opts.bounds[2], opts.bounds[3]])
    })
  ...
  gaodeMap.CoordinateConvert.prototype = {
    ...
    // before：需要转换的坐标类型
    // after: 转换后的坐标类型
    _translate: function(pointArray, before, handler) {
      // 最多10个点
      if (this) {
        let bPoints = []
  ...
  gaodeMap.FontIconMarker = function(map, opts) {
    ...
    if (arr.indexOf(markerOpts.extData.precinctKind) >= 0 && markerOpts.extData.waitHandel > 0) {
      Object.assign(iconLabel, {
        innerHTML:
          '<i class="' +
          markerOpts.getClassnamesOfFontIcon(markerOpts.iconFontClass, markerOpts.fontIcon) +
          '"></i><sup class="waitHandel">' +
          markerOpts.extData.waitHandel +
          '</sup>'
      })
    } else {
      Object.assign(iconLabel, {
        innerHTML:
          '<i class="' + markerOpts.getClassnamesOfFontIcon(markerOpts.iconFontClass, markerOpts.fontIcon) + '"></i>'
      })
    }
    ...
    if (label) {
      ...
      if (label.content) {
        ...
        markerCustomLabelDom.innerHTML =
          "<div style='text-align: center; color: #000000;font-weight:bold;position: relative;font-size: 11px;text-shadow: #FFFFFF 1px 0 0, #FFFFFF 0 1px 0, #FFFFFF -1px 0 0, #FFFFFF 0 -1px 0;white-space:nowrap;'>" +
          label.content +
          '</div>'
      }
      if (label.className) {
