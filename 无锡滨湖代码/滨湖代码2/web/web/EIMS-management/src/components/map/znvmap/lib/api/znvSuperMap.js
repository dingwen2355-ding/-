...
function znvSuperMap (mapServeUrl) {
  ...
  let drawVectorLayer, markerlayer, clusterLayer, heatMapLayer, vector, vectorCircle, textLayer
  let measureDistanceLayer, measureAreaLayer
  // let url = 'http://10.45.148.82:8090/iserver/services/map-zmb/rest/maps/zmb'
  // let url = 'https://iserver.supermap.io/iserver/services/map-china400/rest/maps/China_4326'
  let url = mapServeUrl || 'http://10.45.157.223:8090/iserver/services/map-china400/rest/maps/China_4326'
  let style = {
  ...
  superMap.Map = function (mapDiv, opts, callback) {
    ...
    tiandituLayer = new SuperMap.Layer.WMTS({
      name: "ditu",
      // url: "http://t0.tianditu.gov.cn/vec_c/wmts?tk=1d109683f4d84198e37a38c442d68311",
      // layer: "vec",
      url: sysConfig.map.mapIp + "tileservice/SDPubMap", // 山东天地图
      layer: "cva",
      style: "default",
      matrixSet: "c",
      format: "tiles",
    ...
    tianMarkerLayer = new SuperMap.Layer.WMTS({
      name: "vec",
      url: "http://t0.tianditu.gov.cn/cva_c/wmts?tk=1d109683f4d84198e37a38c442d68311",
      layer: "cva",
      style: "default",
      matrixSet: "c",
      format: "tiles",
    ...
    //     map.addLayers([tiandituLayer, tianMarkerLayer, vector, vectorCircle, clusterLayer, markerlayer])
    //     // 显示地图范围
    //     if(opts.center) {
    ...
    //   }
    // })
    map.addLayers([tiandituLayer, vector, vectorCircle, clusterLayer, markerlayer, textLayer])
    // 显示地图范围
    if(opts.center) {
      opts.center = [opts.center.lng,opts.center.lat];
    }
    ...
    _selectPolygon: function (markers, cb, opts) {
      ...
      drawPolygon.events.on({
        'featureadded': (drawBoundsArgs) => {
          ...
          drawPolygon.deactivate()
          // cb(feature.geometry)
          let polygonPoints=feature.geometry.getVertices();
          let polygonLine = new SuperMap.Geometry.LinearRing(polygonPoints);
          let polygon = new SuperMap.Geometry.Polygon([polygonLine]);
          let resultMarkers = [];
          for (let i = 0; i < markers.length; i++) {
            let pt = markers[i]._getPosition()
            if (polygon.intersects(pt.geometry)) {
              resultMarkers.push(markers[i])
            }
          }
          map.removeLayer(drawVectorLayer)
