 ...
 *半球雷达描圈
 * */
function addEllipsoidScan(viewer, data) {
  // 创建半球
  viewer.entities.add({
    position: Cesium.Cartesian3.fromDegrees(data.lon, data.lat),
    ellipsoid: {
      radii: new Cesium.Cartesian3(data.r, data.r, data.r), // 球的长宽高
      maximumCone: Cesium.Math.toRadians(90),
      material: data.scanColor.withAlpha(0.3),
      outline: true,
      outlineColor: data.scanColor.withAlpha(0.5),
      outlineWidth: 1,
    },
  });
  ...
  var heading = 0;
  var positionArr = calcPoints(data.lon, data.lat, data.r, heading);
  viewer.entities.add({
    wall: {
      positions: new Cesium.CallbackProperty(() => {
        return Cesium.Cartesian3.fromDegreesArrayHeights(positionArr);
      }, false),
      material: data.scanColor.withAlpha(0.5),
    },
  });
  // 执行动画效果
  viewer.clock.onTick.addEventListener(() => {
    heading += data.addHeading; //速度
    positionArr = calcPoints(data.lon, data.lat, data.r, heading);
  });
}
