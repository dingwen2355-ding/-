<template>
  <div class="Right1 module">
    <SmallTitle :titleList="['事件动态']" />
    <img
      class="img"
      v-if="event.length > 0"
      src="../../../../assets/global/排序.png"
      alt=""
      @click="sortList(activeAble)"
      style="top: 8rem; z-index: 2; right: 80rem"
    />
    <div class="content">
      <div style="height: 95%; overflow: auto">
        <div class="content-1" v-for="(item, i) in event" :key="i">
          <div v-show="event.length > i + 1" class="line"></div>
          <div style="display: flex; line-height: 52rem">
            <p class="circle"></p>
...
export default {
  name: 'Right1',
  components: { SmallTitle, VideoConferencingDialog },
  props: ['info'],
  data() {
    return {
      event: [],
