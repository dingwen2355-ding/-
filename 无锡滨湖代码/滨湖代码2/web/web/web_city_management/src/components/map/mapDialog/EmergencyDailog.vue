<template>
  <el-dialog
    title="应急物资仓库"
    :visible.sync="dialogVisible"
    :modal="false"
    width="1431rem"
    height="900rem"
    class="EmergencyDailog  default-dialog-class large-screen"
    :before-close="handleClose"
  >
    <div class="dd-title-back">
      <div class="dd-title-back-s"></div>
    </div>
    <div class="dd-back-area top-right-area"></div>
    <div class="dd-back-angle top-left-angle"></div>
    <div class="dd-back-angle top-right-angle"></div>
    <div class="dd-back-angle bottom-left-angle"></div>
    <div class="dd-back-angle bottom-right-angle"></div>
    <div class="box-1">
      <ul>
        <li>
          <span>地区：</span>
          <span>{{ detailInfo.region }}</span>
        </li>
        <li>
          <span>储备库名称：</span>
          <span>{{ detailInfo.stockName }}</span>
        </li>
<!--        <li>
          <span>仓储面积：</span>
          <span>{{ detailInfo.KR }}</span>
        </li>-->
        <li>
          <span>分管领导：</span>
          <span>{{ detailInfo.mainPeople }}</span>
        </li>
        <li>
          <span>分管领导联系方式：</span>
          <span>{{ detailInfo.dutyPhone }}</span>
        </li>
        <li>
          <span>负责人：</span>
          <span>{{ detailInfo.manager }}</span>
        </li>
        <li>
          <span>负责人联系方式：</span>
          <span>{{ detailInfo.managerPhone }}</span>
        </li>
        <li>
          <span>备注：</span>
          <span>{{ detailInfo.introduction }}</span>
        </li>
      </ul>
    </div>
  </el-dialog>
</template>

<script>
export default {
  components: {},
  props: ['dialogVisibleFlag', 'infomation'],
  watch: {
    dialogVisibleFlag(v) {
      this.dialogVisible = v
      this.detailInfo = this.infomation
    }
  },
  data() {
    return {
      dialogVisible: false,
      detailInfo: {}
    }
  },
  methods: {
    handleClose() {
      this.dialogVisible = false
      this.detailInfo = {}
      this.$emit('update:dialogVisibleFlag', false)
    }
  }
}
</script>

<style lang="scss" scoped>
  .EmergencyDailog {
    ::v-deep .el-dialog{
      width: 2912rem !important;
      height: 1304rem !important;
    }
    .box-1 {
      ul {
        display: flex;
        flex-wrap: wrap;

        li {
          width: 100%;
          margin-left: 10%;
          margin-bottom: 24rem;

          span {
            font-size: 64rem;
            font-family: Microsoft YaHei;
            font-weight: 400;
            line-height: 64rem;
            color: #fff;

            &:nth-of-type(1) {
              color: #3df8f8;
            }

            &:nth-of-type(2) {
              color: #ffffff;
            }
          }
        }
      }
    }
  }
</style>
