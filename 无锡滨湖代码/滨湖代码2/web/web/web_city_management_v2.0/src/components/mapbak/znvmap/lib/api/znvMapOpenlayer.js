...
function openlayerMap(mapServeUrl) {
  ...
  openlayerMap.Layer = function (map, name, opts) {
    ...
    let source = new ol.source.Vector({ wrarem: false })
...
// Marker 使用feature替代overlay
  openlayerMap.OverlayMarker = function (map, opts) {
    ...
    if (opts.icon) {
      ...
      this.textdiv.style.border = '1rem solid gray'
      this.textdiv.style.background = 'white'
      this.textdiv.style.fontSize = '14rem'
      this.textdiv.style.lineHeight = '18rem'
  ...
  openlayerMap.OverlayMarker.prototype = {
    ...
    _setBigIcon: function (icon) {
      ...
      imgdiv.style.top = '-10rem'
      imgdiv.style.width = '30rem'
      imgdiv.style.height = '50rem'
      // this.setElement(imgdiv);
      if (imgdiv) {
        imgdiv.style.top = '-56rem'
        imgdiv.style.fontSize = '18rem'
      }
    },
    /*_setLabel: function (label) {
      ...
      let left = (of && of.x) || 0
      let top = (of && of.y) || 0
      let labelContent = `<div class="amap-marker-label" style="left: ${left}rem;top: ${top}rem">${label.content}</div>`
    ...
    _setLabel: function (label) {
      ...
      if(labelDOM.length > 0) {
        ...
        let labelContent = `<div class="amap-marker-label" style=" position: absolute; left: ${left}rem;top: ${top}rem">${label.content}</div>`
        imgdiv.append(labelContent)
      }
    },
    _setLabel2: function (label) {
      ...
      // map.addOverlay(text);
      let imgdiv = this.getElement()
      let label2 = "<div style='top: -24rem; text-align: center; color: #fff; position: relative;font-size: 11rem;'>" + label + '</div>'
  ...
  openlayerMap.MapLib.MouseTool.prototype = {
    ...
    _selectPolygon: function(markers, cb, opts) {
      ...
      const source = new ol.source.Vector({ wrarem: false })
