...
public class RequestFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest,
        ...
        if (StringUtils.isEmpty(accessToken) && StringUtils.isEmpty(authorization)) {
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                    "Cauthorization error");
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            return;
        }
        if (!StringUtils.isEmpty(accessToken) && Boolean.TRUE.equals(redisService.hasKey(accessToken))) {
            //延长过期时间
            redisService.expireThirdToken(accessToken);
            filterChain.doFilter(servletRequest, servletResponse);
            return;
        }else if(!StringUtils.isEmpty(authorization) && Boolean.TRUE.equals(redisService.hasSelfKey(authorization))){
            //延长过期时间
            redisService.expireSelfToken(authorization);
            filterChain.doFilter(servletRequest, servletResponse);
            return;
        }
