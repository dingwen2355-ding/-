...
public class CommonUtils {
     ...
     * @return
     */
    public static String desensitization(String phones){
        ...
        for (String phone : split) {
            if(StringUtils.isEmpty(phone)){
                if(phone.length()>=4&&phone.length()<=6){
                    String result=phone.substring(0,1);
                    for(int i=0;i<phone.length()-2;i++){
                        result=result+"*";
                    }
                    strings.add(result+phone.substring(phone.length()-1,phone.length()));
                }else if(phone.length()>=7){
                    int length = phone.length();
                    int i=length/2-2;
                    int j=length/2+2;
                    strings.add(phone.substring(0,i)+"****"+phone.substring(j));
                }
            }
        }
        return divisionList(strings);
    }
     ...
     * @return
     */
    public static String matchCallNum(String phones,String callNum){
        //解码
        String telephones = AESUtil.decrypt(phones, "dhf0049001262");
        System.err.println(phones+"---"+callNum);
        String[] split = telephones.split(",");
        for (String s1 : split) {
            if(s1.length()==callNum.length()){
                if(s1.length()>=4&&s1.length()<=6){
                    //判断最后一个数字是否相同
                    if(s1.substring(s1.length()-1).equals(callNum.substring(callNum.length()-1))){
                        return s1;
                    }
                }if(s1.length()>=7){
                    //判断最后两位个数字是否相同
                    if(s1.substring(s1.length()-2).equals(callNum.substring(callNum.length()-2))){
                        return s1;
                    }
                }
            }
     ...
     * @return
     */
    public static String convertEmoji(String s){
    ...
    public static boolean isDateTime(String dateTime){
        String dateTimeReg ="\\d{4}(\\-|\\/|.)\\d{1,2}\\1\\d{1,2} ([0-1]?[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$";
        Pattern pat = Pattern.compile(dateTimeReg);
