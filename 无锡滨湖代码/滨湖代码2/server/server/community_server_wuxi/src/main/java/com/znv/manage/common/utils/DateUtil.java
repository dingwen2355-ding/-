...
public class DateUtil {
    ...
    public static final String formatYMDHmsTZ="yyyyMMdd'T'HHmmss.SSS'Z'";
     ...
     * 时间前推或后推分钟,其中JJ表示分钟.
     */
    public static long getPreTime2(int jj) {
        try {
            Date date = new Date();
            long time = date.getTime() + jj * 60*1000;
            return time;
        } catch (Exception e) {
        }
        return 0L;
    ...
    public static Map<String, String> getStringZeroandEnd() {
    ...
    public static List<Map<String, String>> getTimeList(Date happenTime) {
        List<Map<String,String>> timeList = new ArrayList<>();
        Map<String,String> startMap = new HashMap<>();
        startMap.put("beginTime",String.valueOf(happenTime.getTime()));
        long time = (happenTime.getTime() / 1000) + 3 * 60 * 60;
        happenTime.setTime(time * 1000);
        long last_time = (happenTime.getTime() / 1000) + 24 * 60 * 60;
        Date lastDate = new Date(last_time * 1000);
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:00:00");
        String dateString = formatter.format(happenTime);
        Date newDate = DateUtil.strToDateLong(dateString);
        startMap.put("endTime",String.valueOf(newDate.getTime()));
        timeList.add(startMap);
        Date now = new Date();
        int i = 0;
        while (newDate.compareTo(now) < 0 && newDate.compareTo(lastDate) < 0){
            Map<String,String> lastMap = timeList.get(i);
            Map<String,String> temp = new HashMap<>();
            temp.put("beginTime",lastMap.get("endTime"));
            long temp_time = (newDate.getTime() / 1000) + 3 * 60 * 60;
            newDate.setTime(temp_time * 1000);
            temp.put("endTime",String.valueOf(newDate.getTime()));
            timeList.add(temp);
        }
        return timeList;
    }
    ...
    public static void main(String[] args) {
        ...
        String str="20220428T062606.428Z";
        sdf_input.setTimeZone(TimeZone.getTimeZone("UTC"));
